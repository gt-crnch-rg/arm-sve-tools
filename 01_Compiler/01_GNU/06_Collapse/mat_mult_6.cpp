...
void block_multiply_collapse(double **matA, double **matB, double **matC, unsigned int n,
        unsigned int m, unsigned int l, unsigned int blockSize)
{
    timer_clock::time_point t1= timer_clock::now();
    unsigned int i, j, jj, k, kk;
...
#pragma omp parallel for collapse(2) private(i, j, jj, k, kk)
    ...
    timer_clock::time_point t2= timer_clock::now();
    std::chrono::duration<double> time_span= std::chrono::duration_cast<std::chrono::duration<double>>(t2 - t1);
    printf("Block multiply (loop collapsing) took: %.18lf seconds\n", time_span.count());
}
...
#pragma omp parallel for private(i, j)
    for (i= 0; i < n; ++i){
        for (j= 0; j < m; ++j){
            matA[i][j]= ((double)uniform_dist(el)) / maxRandNum;
        }
...
#pragma omp parallel for collapse(2) private(i, j)
    for (i= 0; i < m; ++i){
        for (j= 0; j < l; ++j){
            matB[i][j]= ((double)uniform_dist(el)) / maxRandNum;
        }
    }
    ...
    // Perform the matrix-matrix multiplication with a bit of blocking and
    // some simple OpenMP paramter changes
    printf("Performing blocked multiply with loop collapsing\n");
    block_multiply_collapse(matA, matB, matC, n, m, l, blockSize);
